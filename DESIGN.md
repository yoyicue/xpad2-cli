...
状态：v0.4.2 已实现；v0.1.1 完成 BoomInstaller 依赖身份和公开分发材料，v0.1.2
...
-> 构建新的 xpad2
-> /260 真机 install full/suu-full、重跑、重启恢复和失败收尾验证
-> 发布新的 XPad2 版本
```
...
10. 幂等收敛正式 0044 anchor，并验证 attribution 与 UID 10072
...
- BoomInstaller 还必须验证激活后的唯一服务为 Root UID 0 或 ADB-shell UID 2000，并验证
  自启动配置；UID 1000/10072 服务或多个并存服务均视为损坏。
- 首次安装和同包同证书升级都走 auto：使用持久 UID 10072/0044 身份，并在该身份内从
  OEM provider 转向 direct。若 0044 缺失或损坏，31317 只补建正式 anchor/alias；外层
  复验 UID 10072 后才开始目标安装。修复失败时不得把目标 APK 交给临时 Root 或 31317。
...
```json
{
  ...
  "version": "0.2.3",
  "kind": "cli",
  "sha256": "e189d0fdbcfbf0d8fc73b9f96fd7afe692c91b92794b6d1b9b270286db1258c8",
  "size": 87208,
...
| `xpad-install` | v0.2.3 | `e189d0fdbcfbf0d8fc73b9f96fd7afe692c91b92794b6d1b9b270286db1258c8` |
| BoomInstaller | v13.6.0.r12.e062328 production | `88495848265a802ad4a1f75b19faa4654959b21261e79d48c7b4615ff3c670e8` |
...
5. 精确 `/260` 普通启动现场可以执行默认 `install`/`full` 或显式 `suu-full`。
