...
状态：v0.3.0 已实现；v0.1.1 完成 BoomInstaller 依赖身份和公开分发材料，v0.1.2
...
anchor、状态机、自动恢复和跨 PackageManager 重写验证（2026-07-15）。
...
| `installer-backup` | policy | 正式 anchor attribution 完整且 `znxrun` 为 UID 10072 | 持久 |
...
9. 幂等收敛正式 0044 anchor，并验证 attribution 与 UID 10072
10. 安装或升级锁定版本的 KernelSU Manager APK
11. 安装 BoomInstaller APK
12. 激活 BoomInstaller 并配置普通开机自启动
13. 所有 APK 事务结束后再次幂等收敛并验证 0044 anchor
14. 验证 OTA、KSU 与全部制品目标状态
...
- 重新计算目标文件 SHA-256。
- 对 `xpad-install` 运行固定的 `doctor`/版本探针，并验证 `installer-backup` 状态。
...
- 首次安装和同包同证书升级都走 auto：先使用持久 UID 10072/0044 身份，再在该身份内
  从 OEM provider 转向 direct。只有 0044 整体失败才使用已有临时 Root，31317 始终是
  最后兜底；31317 成功提交后必须立即补回并复验 0044。`/260` 的 OEM provider 可能
  接收更新请求后不实际提交，auto 必须识别未提交结果并继续，而不能误报成功。
...
```json
{
  ...
  "version": "0.2.1",
  "kind": "cli",
  "sha256": "60505b4b303422f1f3d664b6e1921f5cb59c5c31314e9bdb27cb17b2aa78b980",
  "size": 74600,
...
| `xpad-install` | v0.2.1 | `60505b4b303422f1f3d664b6e1921f5cb59c5c31314e9bdb27cb17b2aa78b980` |
| BoomInstaller | v13.6.0.r11.29ec1f4 production | `2d6ff97da9dc504fb6a236ea8bbd0e39ef861ed2b83da69f76f25679b18b21e0` |
...
18. BoomInstaller 使用标准 Shizuku 权限发现现有客户端，升级时旧授权全部回到待确认。
19. `installer-backup` 只有在正式 anchor attribution 和 `run-as` UID 10072 同时正确时才为 active。
