...
控制面只接受签名锁定的 XPad2 产品族：固定 Android 构建前缀、`release-keys` 后缀、
canonical 数字 incremental 和 arm64 ABI。IonStack Root 再单独要求 incremental 位于
`/19`–`/260` 且内核属于 `4.19.191` 系列。精确 `uname -v` 直接选择 `xpad2-v19-a`、
`xpad2-v19-b` 或 `xpad2-v260`；范围内未知 build 不在入口被拦截，而是进入只读
discovery，以至少两个独立 offset anchor 唯一选型，再依次通过 preflight、validate 和
Root 状态。
它不提供桌面 GUI、桌面 CLI 或 `xpad2.apk`。
...
“当前指纹不能 Root”不等于 `xpad2` 整体不可用。v0.5.2 起，门禁按能力拆开：

- 产品族门：固定 XPad2 fingerprint 前缀、`:user/release-keys` 后缀、canonical 数字
  incremental 和 arm64 ABI；用于自更新、OTA 冻结/解冻、CLI/APK 安装及 0044 修复；
- Root 门：在产品族门之上再要求 incremental `/19`–`/260` 和 `4.19.191` 内核；只用于
  `root`、`ksu`、`suu`、`full`、`suu-full` 和真正启动 IonStack 的路径；
- `/19`–`/260` 内未知 `uname -v` 会进入 POC 的只读 discovery 状态机，不会因未命中
  三个精确 build 而被提前拦截；只有 discovery/preflight/validate 证据不足时才安全停止。

同一 XPad2 产品族但不满足 Root 门时，仍可显式安装完整的无 Root 控制面：

```sh
xpad2 update --check
xpad2 freeze ota
xpad2 install xpad-installer installer-backup ksu-manager boominstaller
```

也可以选择 `suu-manager`，或使用 `xpad2 install cli FILE`、`xpad2 install apk FILE`。
Manager APK 只是管理界面；没有已激活的 KSU/SUU 驱动时，它不会凭空提供 Root。
无参数 `xpad2 install` 等价于包含 KSU 的 `full`，因此 Root 门不满足时必须改用上面的
显式无 Root 组件列表，不能期待 `full` 自动降级或误报部分成功。
...
- `/data/local/tmp/xpad-install` v0.2.14 锁定构建；
- `installer-backup`：正式签名的无代码/无权限 anchor 持久保存 0044 attribution，
  `run-as znxrun` 独立验证为本机 OEM installer 的真实 UID；
- BoomInstaller v13.6.0.r24.2f6e7c2：内部 Shizuku 类/AIDL/Binder 协议保持上游结构，
  Android 全局权限改用 Boom 自有 namespace，可与官方 Shizuku 包共存；
  APK 自带并校验 `xpad-install` v0.2.14；每次安装使用独立的短生命周期 APK/DEX 暂存文件，
  上次失败遗留的只读文件不会阻断下一次安装；Magisk、KernelSU 或 SukiSU Ultra 授权时保持
  真实 UID 0 broker，并通过学而思 OEM Provider 安装。ADB-shell broker 继续使用受管
  0044；两条路径都不依赖 `/data/local/tmp/xpad-install` 或 `xpad2`。普通开机以有界 Job 恢复本地 ADB，不会触发
...
### NeoZygisk / Vector（LSPosed）

Hook 运行时保持显式 opt-in，不进入默认 `full`。未显式选择 runtime 时继续默认 KSU；
v0.6.1 同时支持已经真机验证的显式 SUU 路径：

```sh
xpad2 install zygisk          # 自动包含 ksu
xpad2 install lsposed         # 自动包含 ksu + zygisk
xpad2 install suu zygisk      # 显式选择 SUU + NeoZygisk
xpad2 install suu lsposed     # 显式选择 SUU + NeoZygisk + Vector
xpad2 hooks activate          # 已安装模块的重新激活/修复入口
xpad2 verify zygisk
xpad2 verify lsposed
```

`install zygisk/lsposed` 会在未选择 runtime 时补入 KSU；若同一请求已经显式选择 SUU，
则保持 SUU 且不会再混入 KSU。两条路径都会在同一事务内自动完成 userspace soft-reboot
和严格验活。若该
事务通过 IonStack 创建了临时 Root，live rule 写入后会先恢复 SELinux Enforcing，但保留
临时 `su` 作为恢复通道；Vector Bridge 在 Enforcing 下启动并严格验活后，才关闭
daemon/socket/client。

LS12 的 4.19 late-load KSU 不支持运行时 `SET_SEPOLICY`，但 KSU 域可以加载完整 live
policy。xpad2 因此把 NeoZygisk 工作目录迁到 `/dev/neozygisk`，并把 daemon socket 与模块
memfd 重标为 KSU 已存在的 `ksu_file`；同时用锁定的官方 MagiskPolicy v30.7 只增加 Vector
明确需要的 `system_server self:process execmem`，不导入上游宽泛策略。两套上游
`sepolicy.rule` 都保留为禁用副本，dex2oat 增强仍不在支持范围。

Vector 的自动 system_server 重启次数被固定为 0，避免失败时形成重启风暴。状态验活要求
模块版本、单实例 daemon、Zygote maps、模块 memfd 标签、live policy、当前 system_server
maps 和 Bridge 注入日志。xpad2 不携带或修改任何具体应用的 Hook，不管理应用 scope，
也不把应用进程或业务日志作为 Vector 的成功条件。
回滚使用 `xpad2 hooks disable`，它会移除最小 live rule 并调度 userspace soft-reboot。
普通重启会丢失当前 late-load KSU，不是 Hook 激活/停用手段。
...
后台完成模块注册；xpad2 会保留临时 Root 窗口并等待最多 30 秒，通过完整身份验证后，
再用实际 `ksud module list` 等待最多 15 秒确认控制面可执行。随后特权命令切换到 KSU
的 `/system/bin/su`；临时 Root 只作为恢复通道保留到最终验活，并在恢复 SELinux 后清理。
...
xpad2 install ksu|suu|zygisk|lsposed|ksu-manager|suu-manager|xpad-installer|installer-backup|boominstaller ...
...
更新清单、目标 ELF、catalog 签名、XPad2 产品族和 `/19`–`/260` Root profile 均由内置
production RSA 公钥及 SHA-256 验证。自更新只要求产品族门，不要求当前固件能够 Root。
v0.4.6 起还会读取独立签名的 `xpad2-deltas.json`：只有当前 ELF 的版本、
...
xpad2 update --version 0.6.1     # 选择精确发布版本
...
当设备属于 XPad2 产品族，但 Root profile、动态 discovery 或 PFN/content gate 无法通过时，
使用 [`xpad2-ionstack-poc`](https://github.com/yoyicue/xpad2-ionstack-poc/releases/latest)
host controller 的两个诊断模式。它们不是两套独立二进制；同一个控制器通过参数选择阶段。
先安装官方 Android SDK Platform Tools、连接已授权的 USB 调试设备，并保持 Release 解压后的
`build/` 目录完整：
...
```sh
# 只核对设备、fingerprint、内核和 profile，不进入利用链
./xpad2-ionstack-reroot -s SERIAL --preflight-only

# 运行 discovery 与 order-3/PFN/content gates，不执行最终 adjust-PI/kernel write
...
只有一台 ADB 设备时可以省略 `-s SERIAL`。控制器默认把完整输出保存到
`results/YYYY-MM-DD/reroot_YYYYMMDD_HHMMSS/reroot.log`；远程诊断时应同时提供该文件、
日志开头/结尾的 Boot ID，以及 `xpad2 logs export` 生成的 `xpad2log-*.zip`。

`--validate-only` 虽不执行最终 kernel write，仍会操作漏洞相关的 reclaim/futex 状态，设备
仍可能重启或 kernel panic；它不是零风险检查。不要添加 `--force`，不要把完整 Root 命令
当作诊断步骤，也不要脱离 controller 单独运行 `ionstack_perf_target`、chainwalk probe、
runner 或 preload 等内部制品。普通使用者只需要 `xpad2 doctor` 与 `xpad2 logs export`。
...
仓库的固定输出位置读取。如果共享 PoC checkout 正在其他分支开发，可设置
`XPAD2_IONSTACK_SOURCE=/path/to/release/xpad2-19-260/worktree` 使用隔离的锁定 worktree，
无需覆盖当前工作目录。普通安装完全离线；只有显式 `xpad2 update` 才通过内置
...
`tools/sign_catalog.sh` 默认从 `~/.android/keys` 读取加密托管材料，并用 `~/.ssh` 中固定
指纹的恢复密钥在发布机内存中恢复 PKCS12 密码和临时私钥，随后销毁临时目录。需要直接
从已挂载冷备签名时可设置 `XPAD2_RELEASE_SIGNING_BACKUP=/path/to/protected/signing-backup`，
该模式还会先校验冷备的完整 `SHA256SUMS`。两种模式都固定证书、恢复公钥和 catalog
公钥身份；本地路径可通过 `XPAD2_RELEASE_KEY_DIR`、`XPAD2_RELEASE_RECOVERY_KEY` 与
`XPAD2_RELEASE_RECOVERY_PUBLIC_KEY` 显式覆盖。
...
`/260:user/release-keys` 均可进入 Root 技术范围；`/18` 与 `/261` 不可 Root，但仍属于
同一产品族并可使用无 Root 控制面。`/019`、带符号、溢出或附加字符不是 canonical
incremental，连产品族门也会拒绝。
`build_fingerprint` 中精确 `/260` 值继续作为 v0.4.14 及更旧 updater 的兼容锚点；
非 `/260` 设备首次进入该版本线时需要按小白指南手工推送一次 v0.6.1。签名 catalog
...
不同 build 的 runner/preload 不能盲目互换。表内 build 走精确快速路径；表外但仍在
`/19`–`/260` 范围内的 build 会先运行只读 KASLR/offset discovery。只有至少两个独立
anchor 唯一指向同一 profile，且该 runner 的 preflight 与无写入 validate 状态均完成，
控制面才显式允许最终写入；证据不足或候选冲突时安全停止。

V231227 的 timestamp-style `/1703659196` 是 canonical 数字且属于同一签名 XPad2
产品族，因此可以更新、冻结 OTA、安装 `xpad-installer`/`installer-backup`、Manager APK、
BoomInstaller、任意本地 CLI/APK；它不属于 `/19`–`/260` Root 范围，所以 `root`、
`ksu`、`suu`、`full` 和 `suu-full` 会在任何 IonStack 写入前安全拒绝。
