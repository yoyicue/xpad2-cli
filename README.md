...
- `/data/local/tmp/xpad-install` v0.2.13 锁定构建；
- `installer-backup`：正式签名的无代码/无权限 anchor 持久保存 0044 attribution，
  `run-as znxrun` 独立验证为本机 OEM installer 的真实 UID；
- BoomInstaller v13.6.0.r23.ffa4217：内部 Shizuku 类/AIDL/Binder 协议保持上游结构，
  Android 全局权限改用 Boom 自有 namespace，可与官方 Shizuku 包共存；
  APK 自带并校验 `xpad-install` v0.2.13；Magisk、KernelSU 或 SukiSU Ultra 授权时保持
...
xpad2 update --version 0.5.3     # 选择精确发布版本
...
非 `/260` 设备首次进入该版本线时需要按小白指南手工推送一次 v0.5.3。签名 catalog
