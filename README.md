...
- `/data/local/tmp/xpad-install` v0.2.3 锁定构建；
- `installer-backup`：正式签名的无代码/无权限 anchor 持久保存 0044 attribution，
  `run-as znxrun` 独立验证为 UID 10072；
- BoomInstaller v13.6.0.r12.e062328：保留标准 Shizuku API、客户端发现和授权协议；
  控制面只以 Root 或已配对的本地 ADB shell 运行，隔离的 UID 2000 broker 把 APK 交给
  `xpad-install`。普通开机以有界 Job 恢复本地 ADB，不会触发 0044/31317。
...
xpad2 update --version 0.4.2     # 选择精确发布版本
