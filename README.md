...
- `/data/local/tmp/xpad-install` v0.2.4 锁定构建；
- `installer-backup`：正式签名的无代码/无权限 anchor 持久保存 0044 attribution，
  `run-as znxrun` 独立验证为本机 OEM installer 的真实 UID；
- BoomInstaller v13.6.0.r13.33060a0：保留标准 Shizuku API、客户端发现和授权协议；
  APK 自带并校验 `xpad-install` v0.2.4，隔离的 UID 2000 broker 不依赖
