...
Hook 运行时保持显式 opt-in，不进入默认 `full`。当前只支持已经真机验证的 KSU 路径：
...
`install zygisk/lsposed` 会在同一事务内自动完成 userspace soft-reboot 和严格验活。若该
...
xpad2 update --version 0.6.0     # 选择精确发布版本
...
非 `/260` 设备首次进入该版本线时需要按小白指南手工推送一次 v0.6.0。签名 catalog
