...
`4.19.191` 内核系列和 arm64 ABI。精确 `uname -v` 直接选择 `xpad2-v19-a`、
`xpad2-v19-b` 或 `xpad2-v260`；未知 build 不在入口被拦截，而是进入只读 discovery，
以至少两个独立 offset anchor 唯一选型，再依次通过 preflight、validate 和 Root 状态。
...
xpad2 update --version 0.5.1     # 选择精确发布版本
...
非 `/260` 设备首次进入该版本线时需要按小白指南手工推送一次 v0.5.1。签名 catalog
...
V231227 的 timestamp-style `/1703659196` 不属于 `/19`–`/260` 生产范围，因此仍在
技术范围门处拒绝。
