...
KEY_DIR=${XPAD2_RELEASE_KEY_DIR:-"$HOME/.android/keys"}
...
if [[ -n "$BACKUP" ]]; then
  ...
  RECOVERY_KEY=${XPAD2_RELEASE_RECOVERY_KEY:-"$HOME/.ssh/id_rsa"}
  RECOVERY_PUBLIC_KEY=${XPAD2_RELEASE_RECOVERY_PUBLIC_KEY:-"$HOME/.ssh/id_rsa.pub"}
